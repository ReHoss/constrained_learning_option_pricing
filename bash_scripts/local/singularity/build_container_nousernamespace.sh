...
NAME_SINGULARITY_DEFINITION_FILE="control_dde-firedrake_nousernamespace.def"
...
NAME_IMAGE_SIF_FILE="control_dde-firedrake_nousernamespace_uid-${USER_ID}_gid-${GROUP_ID}_hostname-$(hostname).sif"
PATH_SIF_FILE_DIR="$PATH_CONTENT_ROOT"/singularity/images
PATH_SIF_FILE="$PATH_SIF_FILE_DIR"/"$NAME_IMAGE_SIF_FILE"

# For now, the .def file suppose the construction is performed from the content root
