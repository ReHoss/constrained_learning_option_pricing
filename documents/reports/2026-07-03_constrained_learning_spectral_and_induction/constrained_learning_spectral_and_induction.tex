...
\author{R\'emy Hosseinkhan-Boucher}
\date{3 July 2026}
...
The learned solution is a trial solution that meets the terminal datum by
...
\end{equation}
where the extension $\terminalExtension$ has its own $\timePoint$-dependence and
...
hard form has none. The exact-continuation oracle reaches $1.2\times10^{-3}$,
...
\emph{on the whole line}, and telescoping \eqref{eq:recursion} from
$k=\numExerciseDates-1$ downward gives
\begin{equation}\label{eq:telescope}
  \norm{\stageError_0}_{\mathrm{L}^{\infty}(\R)}
  \;\le\;\sum_{k=0}^{\numExerciseDates-1}
    \norm{\solveError_k}_{\mathrm{L}^{\infty}(\R)}
...
$\varphi_{\sigma^{2}\Delta}$, and $\norm{\varphi_{\sigma^{2}\Delta}}_{\mathrm{L}^{1}(\R)}=1$,
so Young's inequality gives
$\norm{\heatSemigroup f}_{\mathrm{L}^{q}(\R)}\le\norm{f}_{\mathrm{L}^{q}(\R)}$ for
every $q\in[1,\infty]$.
...
\begin{remark}[The function space is the whole line; the measured norms are not]\label{rem:whole-line}
The norms in Proposition~\ref{prop:recursion} are taken over $\R$, and the
qualification is not cosmetic. The kernel of $\heatSemigroup$ is supported on all of
$\R$, so $\heatSemigroup$ is a contraction of $\mathrm{L}^{\infty}(\R)$ but is
\emph{not} an operator on $\mathrm{L}^{\infty}$ of a bounded window $\stateSpace$:
one has only
$\norm{\heatSemigroup f}_{\mathrm{L}^{\infty}(\stateSpace)}
\le\norm{f}_{\mathrm{L}^{\infty}(\R)}$, and the telescoping cannot be closed on
$\stateSpace$ alone. (An earlier statement of this proposition restricted the
contraction to $\mathrm{L}^{\infty}(\stateSpace)$; the correction is recorded here and
changes neither the identity \eqref{eq:recursion} nor the proof.)

The measured quantities of Empirical finding~\ref{find:recursion} are, by contrast,
root-mean-square norms over the \emph{evaluation window} --- a different and weaker
functional than the supremum norm over $\R$ that \eqref{eq:telescope} bounds. The
term-by-term decomposition \eqref{eq:recursion} is a pointwise identity and is
therefore verified on the window without qualification; the \emph{bound}
\eqref{eq:telescope}, and with it the reported bound tightness, is a
window-restricted evaluation of a whole-line inequality. Closing that gap requires
either supplying the trial solution with the known far-field asymptotics of the price
outside the window, or replacing $\heatSemigroup$ by the Dirichlet semigroup on
$\stateSpace$ at the cost of a domain-truncation term Gaussian-small in the ratio of
the window margin to the diffusion length $\sigma\sqrt{\Delta}$. Neither route is
taken here, and the reported tightness is to be read with that reservation.
\end{remark}
...
configuration. Injecting a smoothly windowed single spatial spectral component at the first
...
fourth spectral component, and floors at the probe's spectral-leakage level
...
the one-application kernel-truncation defect retained explicitly, is below
...
\begin{itemize}
  \item the \emph{interior} per-stage solve errors are small ---
    ...
    $k\le\numExerciseDates-2$ --- while the top stage, whose datum is the
...
\begin{figure}[htbp]\centering
  ...
  \caption{The damping operator measured on the exact ten-date induction. Left:
    inception gain of a single injected spectral component versus its wavenumber (solid),
...
of fixed area, so its squared norm is
...
represents (hence cancels) every spectral component of wavenumber $\abs{\wavenumber}\le\cutoffWavenumber$
and nothing above. On the circle, with $\pdeOperator=\partial_{\timePoint}+\generatorUnderlying$
and $\terminalExtension=\interpolationCoefficient\payoff$, every spectral component decouples and
...
With $\interpolationCoefficient(\timePoint)=\timePoint/\timeHorizon$ the low spectral components
($\abs{\wavenumber}\le\cutoffWavenumber$) are reproduced exactly, with zero residual;
for the high spectral components ($\abs{\wavenumber}>\cutoffWavenumber$) the free-network
...
which solves $\pdeOperator\solutionField^{\star}=0$ spectral component by spectral component and matches the datum
...
spectral component by spectral component and subtracting the exact spectral component give \eqref{eq:toy}.
\end{proof}
...
decays, because the operator inverse damps the uncancelled high spectral components.
...
\begin{figure}[htbp]\centering
  ...
  \caption{Residual is not error. Left: one high spectral component's residual (solid) and error
...
backward heat equation: the two-component sine
...
(The note writes $\Psi_\theta$ for the trial solution and $\Phi_\theta$ for the free
...
\begin{enumerate}
  ...
  \item no single $\interpolationCoefficient$ annihilates the forcing of both
    spectral components;
  \item with the coefficient matched to the first spectral component
...
so the forcing factor of the $n$-th spectral component is
...
vanishing is a linear scalar ODE with the unique solution stated (i). Two spectral components
would force
...
(ii). Substituting the matched coefficient into the factor of the $f$-th spectral component gives
...
network once the extension reproduces the low spectral component, i.e.\ a velocity--operator
mismatch on the high spectral component. In the ideal-filter model of
...
Proposition~\ref{prop:matched} is the two-component case of a general obstruction:
a scalar coefficient reproduces the exact solution precisely when the datum is
...
\begin{enumerate}
  ...
  \item If $\Sigma$ contains two values $\modeDecayRate{a}<\modeDecayRate{b}$,
    with non-zero coefficients $c_a,c_b$, then for \emph{every} admissible
...
coefficient depending on the spectral component is precisely the ideal filter of
...
its trial solution is $\architectureOutput=\solutionField^{\star}$ exactly, and the
...
constraint: it has $b(\timeHorizon)=0$, so the trial solution at maturity is the
...
from $\solutionField^{\star}$ reaches the full spectral component amplitude). \emph{Second},
...
$\vartheta_3$ solution, whose spectral component amplitudes grow as $\timePoint$ decreases,
satisfies $\pdeOperator\solutionField=-2\modeDecayRate{n}\solutionField\neq0$
spectral component by spectral component --- it solves the anti-diffusive equation
...
coefficient), while the two-component sine at $(c,f)=(0.5,4)$ reaches
...
frequency content. The exponential coefficient matched to the first spectral component (the note's
...
of annihilating the dominant spectral component's forcing
...
\begin{figure}[htbp]\centering
  ...
  \caption{The note's examples verified. Left: $\mathrm{L}^{2}$-in-time norm of
    the forcing of the $f$-th spectral component against $f^{2}-1$ for the linear, matched-exponential,
    and note-family coefficients; the matched coefficient follows the
    frequency-contrast growth of \eqref{eq:leftover} exactly. Right: first-spectral-component
...
(Figure~\ref{fig:floor-variational}): for a single-component datum the optimal
floor is $4.6\times10^{-13}$ and the minimiser coincides with the matched
exponential to $4.7\times10^{-8}$ (part~(i)); across the two-component sweep
...
finding~\ref{find:note}; for the two-component sine the optimal floor ($4.45$)
...
coefficient concentrates its (larger) floor on the single spectral component $f=4$, which
...
\begin{figure}[htbp]\centering
  ...
  \caption{Variational check of the scalar-coefficient floor. Left: proven
    ...
    spectral gap (two-component datum, $\timeHorizon=1$). Right: the optimal
...
\begin{itemize}
  ...
  \item \textbf{H3/H4 induction} (Tables~\ref{tab:induction}--\ref{tab:mscaling},
    ...
    \emph{Reference correction:} the original reference implementation contained a
