...
$c\neq0$ and integer $f\ge2$, and let
$\interpolationCoefficient\in C^{1}$ with
$\interpolationCoefficient(\timeHorizon)=1$. The mode-$n$ component of the
extension forcing is
...
Let $\payoff\in C^{2}$ be bounded with bounded
$\generatorUnderlying\payoff$, let $\solutionField^{\star}$ be the solution of
\eqref{eq:tvp} in the bounded uniqueness class, and let
$\interpolationCoefficient\in C^{1}$ satisfy
...
objective of Definition~\ref{def:learning} vanishes. Among free fields bounded up
to $\timeHorizon$ --- a class containing every network continuous on the closed
strip --- it is, for a sampler of full support, the unique zero of the objective.
