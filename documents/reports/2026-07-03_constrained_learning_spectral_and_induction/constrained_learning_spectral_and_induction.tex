...
(the homogeneous equation has many solutions), so the field settles on a trivial
...
sits far below both channels --- it is cancellable. Figure~\ref{fig:h2-spike}
...
\begin{figure}[htbp]\centering
  ...
  \caption{Resolving the softplus operator channel. Left: the curvature spike in
    real space, which a coarse grid steps over. Right: its spectrum converging to
...
\begin{figure}[htbp]\centering
  ...
  \caption{Inception price. Solid: learned. Dashed (same colour): exact for that
    $\numExerciseDates$. The learned values sit above the European put (a positive
...
chain's larger error is the price of learning, rather than being handed, the
...
accumulate. The learned $\numExerciseDates=10$ inception price sits visibly below
...
\begin{corollary}[The operator channel of a kinked datum is white]\label{cor:white}
For a kink ($\regularityIndex=1$) under a second-order generator
...
\end{equation}
For the kink under the heat operator ($\operatorHalfOrder=\regularityIndex=1$)
...
datum drops to an exact cliff (an exact spectral gap gives an exact solution), a
smooth datum falls exponentially, and a kink leaves the near-flat white plateau
...
kink's $\wavenumber^{-4}$ datum into that plateau. Figure~\ref{fig:toy-rve} shows
...
\begin{figure}[htbp]\centering
  ...
  \caption{Ideal-filter model. Left: the uncancellable residual versus the cutoff
    $\cutoffWavenumber$; the kink is a near-flat plateau. Right: the space-time
    solution error; the kink still decays. Solid curves, colour-coded by datum
...
\begin{figure}[htbp]\centering
  ...
  \caption{The $\wavenumber^{4}$ operator amplification. Datum spectrum (dashed)
    and operator channel (solid): a kink's $\wavenumber^{-4}$ datum becomes a
...
(Chen--Mangasarian) extension the residual sits far below the forcing
