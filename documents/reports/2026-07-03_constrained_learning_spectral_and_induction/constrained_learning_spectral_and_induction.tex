...
\begin{figure}[htbp]\centering
  \includegraphics[width=0.82\textwidth]{figures/2026-07-04_bermudan_error_propagation_seeds.png}
  \caption{Error at each exercise date, maturity (right) to inception (left); seed
    ...
    $\numExerciseDates$; hard convex solid, soft dashed.}\label{fig:h3-prop}
\end{figure}
\begin{figure}[htbp]\centering
...
\caption{Hard convex induction versus number of exercise dates
  ...
  $<5\times10^{-4}$ throughout). Inception error and peak per-date error.}\label{tab:mscaling}
\begin{tabular}{lcc}
\toprule
$\numExerciseDates$ & $\relLtwo$ (inception) & $\relLtwo$ (peak per-date)\\
\midrule
2  & $3.17\times10^{-2}$ & $3.17\times10^{-2}$\\
3  & $3.10\times10^{-2}$ & $5.77\times10^{-2}$ (near maturity)\\
5  & $5.93\times10^{-2}$ & $1.02\times10^{-1}$ (mid-life)\\
...
The close agreement at $\numExerciseDates=2,3$ ($3.17\times10^{-2}$ against
$3.10\times10^{-2}$) does not persist: the inception error then grows monotonically,
$5.93\times10^{-2}$ at $\numExerciseDates=5$ and $1.44\times10^{-1}$ at
$\numExerciseDates=10$, where the per-date error peaks near $1.9\times10^{-1}$ in
mid-life --- roughly a fourfold rise in the inception error, and sixfold in the peak
per-date error, from $\numExerciseDates=2$ to $10$ (Figure~\ref{fig:h3-prop}). Two
effects act in the same direction as $\numExerciseDates$ grows: each interval is
shorter, so its Gaussian propagation smooths the payoff and exercise-boundary
curvature less --- the smoothing length
$\sigma\sqrt{\timeHorizon/\numExerciseDates}$ falls from $0.18$ at
$\numExerciseDates=2$ through $0.11$ at $\numExerciseDates=5$ to $0.08$ at
$\numExerciseDates=10$, raising the per-stage operator channel by
Proposition~\ref{prop:floor} --- and there are more stages to accumulate. The
learned $\numExerciseDates=10$ inception price lies visibly below its exact
reference for large spot. Measured over three seeds; the seed standard deviation is
below $5\times10^{-4}$ at every date and every $\numExerciseDates$, so the growth is
a systematic effect and not seed noise, though its exact rate in $\numExerciseDates$
is not characterised. Hypothesis~H4 is refuted.
\end{observation}
