...
\end{proposition}
\begin{proof}
The displayed norm identity follows from orthonormality and
$\pdeOperator(\interpolationCoefficient\payoff)
=\sum_j c_j(\interpolationCoefficient'-\modeDecayRate{j}\interpolationCoefficient)\basisFunction_j$.
(i) Vanishing requires
$\interpolationCoefficient'-\modeDecayRate{j}\interpolationCoefficient\equiv0$
for every $j$ with $c_j\neq0$; each such equation with
$\interpolationCoefficient(\timeHorizon)=1$ has the unique solution
$e^{\modeDecayRate{j}(\timePoint-\timeHorizon)}$, and two distinct rates give
distinct solutions, so $\Sigma$ must be a singleton; conversely the matched
exponential annihilates the forcing, and
$\interpolationCoefficient\payoff$ coincides mode by mode with the semigroup
solution $e^{(\timeHorizon-\timePoint)\generatorUnderlying}\payoff=\solutionField^{\star}$.
(ii) Set
$F_a=\interpolationCoefficient'-\modeDecayRate{a}\interpolationCoefficient$ and
$F_b=\interpolationCoefficient'-\modeDecayRate{b}\interpolationCoefficient$,
with time norms $A=\norm{F_a}$, $B=\norm{F_b}$. Subtracting,
$(\modeDecayRate{b}-\modeDecayRate{a})\norm{\interpolationCoefficient}
=\norm{F_a-F_b}\le A+B$. Solving
$\interpolationCoefficient'=\modeDecayRate{a}\interpolationCoefficient+F_a$
backward from $\interpolationCoefficient(\timeHorizon)=1$ gives
$\interpolationCoefficient(\timePoint)
=e^{\modeDecayRate{a}(\timePoint-\timeHorizon)}
-\int_{\timePoint}^{\timeHorizon}e^{\modeDecayRate{a}(\timePoint-s)}F_a(s)\,ds$,
and Young's convolution inequality bounds the integral term in
$\mathrm{L}^{2}(0,\timeHorizon)$ by $\kappa_1\norm{F_a}$, so
$\norm{\interpolationCoefficient}\ge\kappa_2-\kappa_1 A$. Combining,
$A+B\ge(\modeDecayRate{b}-\modeDecayRate{a})(\kappa_2-\kappa_1 A)$, hence
$\max(A,B)\ge(\modeDecayRate{b}-\modeDecayRate{a})\kappa_2
/\bigl(2(1+(\modeDecayRate{b}-\modeDecayRate{a})\kappa_1)\bigr)$, and
$\normTimeSpace{\pdeOperator(\interpolationCoefficient\payoff)}
\ge\min(\abs{c_a},\abs{c_b})\max(A,B)$ from the two retained terms of the norm
identity.
\end{proof}
...
\begin{proof}
Recomposing gives
$\architectureOutput=(1-\interpolationCoefficient)\freeNetwork^{\star}+\interpolationCoefficient\payoff=\solutionField^{\star}$,
hence a vanishing residual. At $\timeHorizon$ both
$\solutionField^{\star}-\interpolationCoefficient\payoff$ and
$1-\interpolationCoefficient$ vanish; since
$\partial_{\timePoint}\solutionField^{\star}(\cdot,\timeHorizon)=-\generatorUnderlying\payoff$,
l'H\^{o}pital's rule gives the stated limit, finite because
$\interpolationCoefficient'(\timeHorizon)\neq0$. For uniqueness: a zero of the
objective has $\pdeOperator\architectureOutput=0$ on the support of the sampler;
if the free field is bounded up to $\timeHorizon$ then
$(1-\interpolationCoefficient)\freeNetwork\to0$ there, so
$\architectureOutput(\cdot,\timeHorizon)=\payoff$, and uniqueness of the
terminal-value problem in the bounded class forces
$\architectureOutput=\solutionField^{\star}$, hence
$\freeNetwork=\freeNetwork^{\star}$.
\end{proof}
...
\end{lemma}
\begin{proof}
If the quotient equals $\gamma$ then
$1-\interpolationCoefficient=Ce^{-\gamma\timePoint}$; continuity at
$\timeHorizon$ and $\interpolationCoefficient(\timeHorizon)=1$ force $C=0$, so
$\interpolationCoefficient\equiv1$, contradicting the definedness of the
quotient.
\end{proof}
...
\begin{itemize}
  ...
  \item \textbf{Companion-note examples} (Section~\ref{sec:mech-note},
    Empirical finding~\ref{find:note}, Figure~\ref{fig:note}):
    ...
    \texttt{analysis\_summary.yaml}); executed on Jean~Zay \texttt{prepost}.
    Measured accuracies from the \texttt{sine} / \texttt{theta3} runs
