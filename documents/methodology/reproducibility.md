...
| `--extraction` | flag | *off* | Enable singularity extraction ansatz for Stage B (§2.2 of architecture.md). Decomposes $U_B = v + \tilde{u}_\theta$, removing the $C^0$ kink at the exercise boundary $s^*$. When *off*, Stage B directly interpolates $V_{\text{target}}(s, t_1)$. |
| `--interp {cubic,pchip,linear}` | str | `cubic` | Interpolator used for $V_{\text{target}}(s, t_1)$ in the **non-extraction** path. Ignored when `--extraction` is set. `cubic`: $C^2$ natural spline. `pchip`: $C^1$, shape-preserving. `linear`: $C^0$, drops diffusion (benchmarking only). |
...
| `--bermudan-only` | flag | *off* | Skip the European problem and go directly to the Bermudan stages. |
...
| `etcnn_a.pt` | Saved Stage A network weights |
| `training_metrics/` | Loss curves per model |
| `pricing/` | Price surface plots |
| `greeks/` | Delta / Gamma plots |
| `diagnostics/` | Residuals, exercise boundary, interpolation diagnostics |
