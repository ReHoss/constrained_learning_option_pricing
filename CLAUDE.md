...
- On Jean Zay (and IDRIS clusters in general), **never run computation on the login nodes**. Login nodes are for editing files, submitting jobs, and lightweight I/O only. Building a model, running a forward pass, calling autograd, or anything that allocates more than a few MB of tensors **must** go through a compute node — either an interactive shell (`srun --partition=prepost --time=00:05:00 --pty bash` for short CPU tasks; `--partition=gpu_p13 --gres=gpu:1` for GPU) or a batch job. The launcher in `bash_scripts/cluster/jeanzay/python/ablation_array_launcher.sh` is designed accordingly: its `--init-only` phase on the login node only writes YAML/metadata files (no torch tensors), and every real Python invocation happens inside an sbatch task. When suggesting commands to be run on Jean Zay, default to `srun`/`sbatch` for anything that touches PyTorch beyond `import torch`.
