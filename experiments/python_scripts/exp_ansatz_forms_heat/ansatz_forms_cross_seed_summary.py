            ...
            if os.path.exists(hist_path):
                with np.load(hist_path) as h:
                    if "forcing_floor" in h:
                        agg[ic][v]["forcing_floor"].append(
                            float(np.median(h["forcing_floor"])))
    return agg, run_dirs
...
def _formula_box(fig, text):
    """Add a LaTeX text block stating the plotted quantity below the figure."""
...
def _check_layout(fig, fname, *, legends=(), formula=None, axes=()):
    """Emit warnings for common figure-layout defects.

    1. **Cropping risk** -- any legend / title / x- or y-axis label whose box
       spills past the figure canvas (it is rescued by ``bbox_extra_artists``
       on save, but the spill means the layout under-reserves space).
...
def _finalize(fig, path, *, legends=(), formula=None, axes=()):
    """Add the formula box, check the layout, and save including all legends."""
    fname = os.path.basename(str(path))
    legs = [lg for lg in legends if lg is not None]
    formula_art = _formula_box(fig, formula) if formula else None
    extra = legs + ([formula_art] if formula_art is not None else [])
    _check_layout(fig, fname, legends=legs, formula=formula_art, axes=axes)
    fig.savefig(path, dpi=140, bbox_inches="tight", bbox_extra_artists=extra)
    plt.close(fig)
...
def plot_rel_l2_by_ic(agg, out_dir):
    ...
    leg = ax.legend(loc="upper left", bbox_to_anchor=(1.02, 1.0), fontsize=7, frameon=True)
    fig.tight_layout(rect=[0, 0.11, 0.80, 1])
    _finalize(
        fig, out_dir / "rel_l2_by_ic.png", legends=[leg], axes=[ax],
        formula=(r"$\mathrm{rel}\,L^2 = \|\hat{u}-u^\star\|_{L^2"
                 r"(\mathcal{X}_{\mathrm{eval}}\times[0,T])}\,/\,\|u^\star\|_{L^2}$, "
                 r"mean $\pm$ std over seeds; $u^\star$ exact, $\hat u$ trained"),
    )
...
def plot_terminal_mismatch(agg, out_dir):
    ...
    fig.tight_layout(rect=[0, 0.11, 0.80, 1])
    _finalize(
        fig, out_dir / "terminal_mismatch.png", legends=[leg], axes=[ax],
        formula=(r"$\mathrm{rel}\,L^2_{t=T} = \|\hat{u}(\cdot,T)-g\|_{L^2}"
                 r"\,/\,\|g\|_{L^2}$  (terminal datum $g$; hard forms $=0$ by "
                 r"construction, floored at $10^{-6}$ for the log axis)"),
    )
...
def plot_floor_vs_accuracy(agg, out_dir):
    ...
    ax.add_artist(leg1)
    leg2 = ax.legend(handles=ic_handles, loc="lower left", bbox_to_anchor=(1.02, 0.0),
                     fontsize=7, frameon=True, title="IC")
    fig.tight_layout(rect=[0, 0.12, 0.78, 1])
    _finalize(
        fig, out_dir / "floor_vs_accuracy.png", legends=[leg1, leg2], axes=[ax],
        formula=(r"$x=\mathbb{E}_\mu[(\mathcal{P}\Psi)^2]$ (median over training); "
                 r"$y=\mathrm{rel}\,L^2$ vs exact.  "
                 r"$\mathcal{P}u=\partial_t u+\frac{\sigma^2}{2}\partial_{xx}u$, "
                 r"$\Psi$ the terminal-data extension"),
    )
